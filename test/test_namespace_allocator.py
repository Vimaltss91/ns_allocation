...
def test_extract_args_yaml(namespace_allocator):
    ...
    with patch('namespace_allocator.extract_from_yaml', return_value=mock_parse_vars_data) as mock_extract_yaml, \
            patch('namespace_allocator.parse_variables', return_value=mock_parse_vars_data) as mock_parse_vars:
...
def test_insert_or_update_status(namespace_allocator, test_data, mock_get_existing_namespace_return, expected_update_call, expected_insert_call):
    ...
    # Ensure that 'namespace' starts with "o-devops-" to match the expected test scenario
    kwargs['namespace'] = "o-devops-" + kwargs['namespace']

    # Correct the patching path to reflect the actual usage in namespace_allocator.py
    with patch('namespace_allocator.get_existing_namespace_hardcoded_ns', return_value=mock_get_existing_namespace_return) as mock_get_existing_namespace, \
            patch('namespace_allocator.update_namespace_status_hardcoded_ns') as mock_update_ns_status, \
            patch('namespace_allocator.insert_new_status') as mock_insert_new_status, \
            patch('namespace_allocator.update_namespace_pool_status') as mock_update_ns_pool:
        namespace_allocator.insert_or_update_status(**kwargs)

        # Assertions to verify the function calls
...
def test_allocate_namespace_with_existing_namespace(namespace_allocator, test_data):
    ...
    with patch('namespace_allocator.get_existing_status', return_value=('namespace', 'ASSIGNED', 'namespace_name', 'pipeline_url')) as mock_get_assigned_status, \
        ...
        # Verify get_assigned_status was called with the correct cursor and kwargs
        mock_get_assigned_status.assert_called_once_with(mock_cursor, kwargs)

        # Since the namespace is assigned, find_and_lock and fetch_total_cpu_requests should not be called
        mock_find_and_lock.assert_not_called()
        mock_fetch_cpu.assert_not_called()

        # Ensure the correct pipeline URL is returned
...
def test_allocate_namespace_no_available_namespace(namespace_allocator, test_data):
    ...
    with patch('namespace_allocator.get_existing_status', return_value=None) as mock_get_assigned_status, \
            patch('namespace_allocator.find_and_lock_available_namespace', return_value=None) as mock_find_and_lock, \
            patch('namespace_allocator.fetch_total_cpu_requests_with_validation', return_value=1000) as mock_fetch_cpu:
        result = namespace_allocator.allocate_namespace(**kwargs)
...
def test_delete_namespace_success(mock_db_connection):
    ...
    with patch('namespace_allocator.delete_namespace_from_status') as mock_delete_namespace, \
            patch('namespace_allocator.update_namespace_pool_status') as mock_update_ns_status:
        allocator.delete_namespace(namespace_name)

        # Update to match the expected call with four arguments
        mock_delete_namespace.assert_called_once_with(mock_cursor, namespace_name)
        mock_update_ns_status.assert_called_once_with(mock_cursor, namespace_name, "Available", "NO")
...
def test_allocate_namespace_exception_handling(namespace_allocator, test_data):
    ...
    with patch('namespace_allocator.get_existing_status', side_effect=Exception('Database error')) as mock_get_assigned_status:
...
def test_insert_or_update_status_empty_input(mock_db_connection, test_data):
    ...
    with patch('namespace_allocator.get_existing_namespace_hardcoded_ns', return_value=None) as mock_get_existing_namespace, \
            patch('namespace_allocator.update_namespace_status_hardcoded_ns') as mock_update_ns_status, \
            patch('namespace_allocator.insert_new_status') as mock_insert_new_status, \
            patch('namespace_allocator.update_namespace_pool_status') as mock_update_ns_pool:
        ...
        mock_update_ns_pool.assert_called_once_with(mock_cursor, kwargs['namespace'], 'IN-USE', 'NO')
