...
def get_upgrade_status(cursor, kwargs: dict) -> tuple:
    """Retrieves the existing status for a namespace from the database."""
    query = """
        SELECT  release_tag, upg_rollback FROM namespace_status
...
def update_status_and_lock(connection, cursor, namespace_name: str, pipeline_url: str, kwargs: dict):
    ...
    try:
        ...
        update_params = (
            ...
            kwargs['is_tgz'], kwargs['is_internal_ats'], kwargs['is_occ'], kwargs['is_pcf'],
            kwargs['is_converged'], kwargs['is_pcrf'], kwargs['upg_phase'], kwargs['play_id'], kwargs['tls_version'], kwargs['upg_rollback'], kwargs['official_build'], kwargs['custom_message']
        )
